...
const Project = ({ project }: ProjectProps) => {
  ...
  return (
    <div
      className={`flex flex-row min-h-screen w-full text-xl justify-center items-center gap-12 px-32`}
      id={bg}
      data-aos="fade-up"
    >
      <div className="">
        <Image src={img} alt={title} width={1000} />
      </div>
      <div className="flex flex-col justify-start gap-6 w-1/2">
        <h4 className="text-5xl uppercase font-bold">{title}</h4>
        <p className="text-sm text-white lg:text-xl leading-8 tracking-wider">{desc}</p>
